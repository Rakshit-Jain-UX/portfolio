<script>
  ...
  gsap.to(h2, {
    ...
    onComplete: () => {
      ...
      gsap.to([link, descriptionEl], { opacity: 1, duration: 0.3 });
    ...
    return ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        if (self.progress > 0 && self.progress < 1) {
          ...
          gsap.to(pElement, { opacity: 0, duration: 0 , display: "none"});
        } else {
          // Show <p> again when the section is out of view
          gsap.to(pElement, { opacity: 1, duration: 0 ,  display: "block"});
        }
      },
    });
  });
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (!isActive) {
        ...
        gsap.to(pElement, { opacity: 1, duration: 0,  display: "block" }); // Ensure <p> is visible when at the top
...
<main class="bg-[#F6F6F1] h-min main">
  ...
  >
    <h2 class="text-[80px] uppercase font-b text-[#fff] leading-tight">
      ...
      <a href="#" class="hidden cursor-target transition-all saol-italic capitalize font-medium" > Projects </a>
    </h2>
    <div
 ...
 <div class="cus-container">
  <div
  class="project-section h-dvh grid sm:grid-cols-2 gap-20px"
  id="project-beta"
