<script>
  import "../../app.css";
</script>

<main class="bg-primary">
  <div
    class="banner cus-container h-dvh relative pt-80 pb-52 text-center flex justify-center flex-col gap-[32px]"
  >
    <span class="text-[18px] text-[#736673]">Hi! I’m Rakshit Jain</span>
    <div class="wrap">
      <div
        class="font-b leading-[90px] flex justify-center flex-wrap uppercase header text-[30px] sm:text-[60px] md:text-[90px] gap-[24px]"
      >
        <span class="flex-center"
          >web designer <img src="https://picsum.photos/100" alt="" /></span
        >
        <div>
          <span class="cursew"><span class="saol-italic lowercase"> based in</span> ahmedabad</span>
        </div>
      </div>
    </div>
    <div
      class="desc mx-auto text-[18px] lg:text-[24px] w-full max-w-[824px] leading-normal text-[#736673]"
    >
      <p>
        5 years delivering solutions across various industries in a consulting
        setting. I pair strong visual design skills with expertise in strategy
        and UX aligned with business objectives. My experience spans diverse
        sectors: retail, tech startups, healthcare, sports, B2B companies,
        institutions…
      </p>
    </div>
  </div>
</main>

<style>
  .banner {
    color: #000;
  }
</style>
